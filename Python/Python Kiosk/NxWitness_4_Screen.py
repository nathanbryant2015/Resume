...
nx_layout03_name = config["nx_layout03_name"]
nx_layout04_name = config["nx_layout04_name"]
screen_0_number = config["Nx_Screen_0"]
screen_1_number = config["Nx_Screen_1"]
screen_2_number = config["Nx_Screen_2"]
screen_3_number = config["Nx_Screen_3"]
...
while True:
    ...
    cmd2 = f'"{nx_shortcut}" --auth=http://{username}:{password}@{server_ip}:7001 --no-fullscreen=false --layout-name "{nx_layout03_name}" --screen={screen_2_number}'
    cmd3 = f'"{nx_shortcut}" --auth=http://{username}:{password}@{server_ip}:7001 --no-fullscreen=false --layout-name "{nx_layout04_name}" --screen={screen_3_number}'

    #Exectures the cmd variables
    ...
    process2 = subprocess.Popen(cmd3, shell=True)
